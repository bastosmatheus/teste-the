...
describe("Teste unitário para criação de uma avaliação no banco de dados", () => {
  beforeEach(() => {
    ...
    deleteRatingService = new DeleteRatingService(ratingRepository);
  });

  test("Deve ser possível fazer a exclusão de uma avaliação", async () => {
    ...
    const rating = await deleteRatingService.execute({
      id: 1,
    });
  ...
  test("Não deve fazer a exclusão se o ID passado não corresponder a uma avaliação", async () => {
    await expect(async () => {
      await deleteRatingService.execute({
        id: 1927831823,
      });
